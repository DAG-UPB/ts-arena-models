...
def predict(request: PredictionRequest):
    ...
    if is_batch:
        ...
        # Get predictions from model - moment expects List[List[float]]
        ...
        # Single series - moment requires List[List[float]], so wrap it
        ...
        predictions = model.predict(history_values, request.horizon)
        prediction = predictions[0]  # Get first (only) series result
...
async def health_check():
    try:
        prediction = model.predict([[1,2,3,4,5,6,7,8]], horizon=2)
        print("Health check prediction:", prediction)
        # Model loading check
