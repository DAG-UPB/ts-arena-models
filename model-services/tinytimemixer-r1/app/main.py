...
from app.model import TinyTimeMixerR1Model
...
async def startup_event():
    global model
    logger.info("Loading TinyTimeMixer R1 model...")
    model = TinyTimeMixerR1Model()
    ...
    try:
        ...
        if is_batch:
            ...
            for i, last_ts in enumerate(last_timestamps):
                ...
                # quantiles is a list when in batch mode
            ...
            return PredictionResponse(prediction=all_forecasts)
        else:
            # Single series
            ...
            logger.info(f"Received prediction request: history_len={len(data)}, horizon={request.horizon}")
            ...
            # Format response
            forecast_items = create_forecast_items(future_ts, forecasts, quantiles)
